...
export default function ListUserDeleted() {
    ...
    useEffect(() => {
        const getNumberPage = async () => {
            try {
                const res = await axios.get(`${BASE_URL}/user/pageDeleted`)
    ...
    useEffect(() => {
        const getUsers = async () => {
            const res = await axios.get(`${BASE_URL}/user/deleted?page=${currentPage}`, {
    ...
    const handleCancelDeleteUserItem = async (id) => {
        ...
        const res = await axios.put(`${BASE_URL}/user/cancel-delete/${id}`,{},
        {
            headers: { Authorization: `Bearer ${localStorage[SUMMER_SHOP]}` }
        })
